...
def line_chart(lab_num,title,x_label,y_label,index_col,font_title,font_xlabel,font_ylabel):
   ...
   #plt.plot(data['value'][239:], label="value", c="b", marker=".", markersize=7, markerfacecolor='none')
    ...
    plt.legend()
    plt.savefig('./Output/lab1.png'.format(lab_num),dpi=1000, bbox_inches='tight')

    plt.show()
#         lab_num,title,x_label,y_label,index_col,font_title,font_xlabel,font_ylabel)   第一个参数是选择excel表的参数
line_chart(1,"","Switch Number","INT Path Number","index",font_title,font_xlabel,font_ylabel)
