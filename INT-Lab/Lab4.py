...
def line_chart(lab_num,title,x_label,y_label,index_col,font_title,font_xlabel,font_ylabel):
   ...
   #40行数据开始
    plt.plot(data['Balance'],label= "Balance",c = "r",marker = "^",markersize=7,markerfacecolor='none')
    plt.plot(data['Unbalance'],label= "Unbalance",c = "b",marker = "o",markersize=7,markerfacecolor='none')
    #设置间隔
    ...
    plt.legend()
    plt.savefig('./Output/lab4.png'.format(lab_num),dpi=1000, bbox_inches='tight')

    plt.show()
#         lab_num,title,x_label,y_label,index_col,font_title,font_xlabel,font_ylabel)   第一个参数是选择excel表的参数
line_chart(4,"","Switch Number","Variance of Path Lengths","index",font_title,font_xlabel,font_ylabel)